...
Let $H_{db,1990}$ be the 1990 owner-occupied share of occupied housing units in community district $d$ in borough $b$.
...
\begin{equation}
    ...
    =
    \frac{
        H_{db,1990} - \overline{H}_{b,1990}^{\mathrm{CD}}
    }{
        s_b(H_{db,1990})
    },
\end{equation}

where $\overline{H}_{b,1990}^{\mathrm{CD}}$ and $s_b(H_{db,1990})$ are the mean and
standard deviation of 1990 homeownership across community districts in
borough $b$.

This score compares each community district to others in the same borough.
...
The within-borough score instead asks whether homeowner-heavy community districts experienced different post-reform construction trajectories than their lower-homeownership neighbors in the same borough.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[page=1,width=0.82\textwidth]{input/cd_homeownership_1990_map.pdf}
    \caption{Community-District Homeownership Exposure}
    \label{fig:treatment_map}
\figurenotes{Figure maps the 1990 community-district homeownership share standardized within borough. The measure uses the Department of City Planning's 1990 community-district profiles. Redder districts had higher homeownership than other community districts in the same borough; bluer districts had lower homeownership.}
\end{figure}
