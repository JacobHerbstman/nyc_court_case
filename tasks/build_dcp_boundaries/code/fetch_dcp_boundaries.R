...
current_boundary_source_ids <- "dcp_boundary_community_districts"
...
if (nrow(boundary_rows) != 1 || boundary_rows$source_id[[1]] != current_boundary_source_ids) {
  stop("Source catalog must contain the scripted DCP community-district boundary row.")
}

dataset_ids <- c(dcp_boundary_community_districts = "5crt-au7u")

download_names <- c(dcp_boundary_community_districts = "community_districts.zip")
