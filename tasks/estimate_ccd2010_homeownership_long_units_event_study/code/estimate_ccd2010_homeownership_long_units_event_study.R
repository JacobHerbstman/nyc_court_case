...
write_csv_if_changed(
  bind_rows(
    tibble(metric = "district_count", value = as.character(n_distinct(design_df$district_id)), note = "2010 Council districts in the PLUTO-only event-study design."),
    ...
    tibble(metric = "source_family_count", value = as.character(n_distinct(design_df$source_family)), note = "Distinct construction-outcome source families in the design."),
    tibble(metric = "source_family", value = paste(sort(unique(design_df$source_family)), collapse = ";"), note = "Construction-outcome source family used in all event-study years."),
    ...
    tibble(metric = "status", value = as.character(as.integer(n_distinct(design_df$source_family) == 1 && unique(design_df$source_family) == "mappluto_proxy_25v4")), note = "One means construction outcomes are PLUTO-only in the event-study design.")
  ),
  "../output/ccdist2010_homeownership_long_units_design_qc.csv"
)
