...
def key_terms(row):
    terms = [
        ...
        row["zap_project_ids"],
        row["zap_project_names"],
        row["matter_files"],
        row["adjudication_affected_council_districts"],
    ]
    for value in [
        row["adjudication_project_name"],
        row["adjudication_project_area"],
...
def search_queries(row):
    ...
    zap_projects = split_values(row["zap_project_ids"])
    ...
    for zap_project in zap_projects[:2]:
        queries.append(f'"{zap_project}" "Council District"')
        queries.append(f'"{zap_project}" NYC ZAP')
    if project_area:
...
def prompt_header(row, queries):
    ...
    return f"""# NYC Council Land-Use Remaining-Queue Web Review

You are helping audit a PhD research dataset of New York City Council land-use decisions.

This is the full-text/web review pass for the 161-signature geography repair universe, excluding the 52 cases already manually reviewed. Decide whether the affected Council district(s) can be promoted for this signature.
...
prior_ai_claimed_districts: {row["adjudication_affected_council_districts"]}
prior_ai_district_source: {row["adjudication_district_source"]}
prior_ai_confidence: {row["adjudication_district_confidence"]}
prior_ai_status: {row["adjudication_status"]}
prior_ai_candidate_category: {row["adjudication_candidate_category"]}
project_area_from_prior_ai: {row["adjudication_project_area"]}
prior_source_check_summary: {row["adjudication_source_check_summary"]}
prior_evidence_limitations: {row["adjudication_evidence_limitations"]}
prior_recommended_researcher_action: {row["adjudication_recommended_researcher_action"]}
...
os.makedirs("../output/remaining_queue_web_review_batches", exist_ok=True)

all_rows = read_csv_rows("../input/council_land_use_missing_geography_adjudication_ai_repair_candidates.csv")
manual_ids = {
    row["signature_review_id"]
    for row in read_csv_rows("../output/manual_queue_web_review_batches/manual_queue_web_review_case_manifest.csv")
}
rows = [row for row in all_rows if row["signature_review_id"] not in manual_ids]
rows = sorted(rows, key=lambda row: int(row["signature_review_id"].split("_")[-1]))
...
for row in rows:
    ...
    case_path = f"../output/remaining_queue_web_review_batches/{row['signature_review_id']}_remaining_queue_web_prompt.md"
    ...
    case_manifest_rows.append(
        {
            ...
            "prior_ai_status": row["adjudication_status"],
            "prior_ai_districts": row["adjudication_affected_council_districts"],
...
for start in range(len(case_prompts)):
    batch_id = f"{len(batch_rows) + 1:03d}"
    batch_cases = case_prompts[start : start + 1]
    batch_text = "\n\n".join(case["prompt"] for case in batch_cases)
    batch_path = f"../output/remaining_queue_web_review_batches/remaining_queue_web_review_batch_{batch_id}.md"
...
write_csv_if_changed(
    ...
    "../output/remaining_queue_web_review_batches/remaining_queue_web_review_case_manifest.csv",
)
write_csv_if_changed(
    batch_rows,
    ["batch_id", "batch_path", "signature_count", "signature_review_ids", "char_count"],
    "../output/remaining_queue_web_review_batches/remaining_queue_web_review_batch_manifest.csv",
)
write_text_if_changed(
    "\n".join(json.dumps(row, ensure_ascii=True) for row in batch_rows) + "\n",
    "../output/remaining_queue_web_review_batches/remaining_queue_web_review_batches.jsonl",
)
write_csv_if_changed(
    query_rows,
    ["signature_review_id", "search_query"],
    "../output/remaining_queue_web_review_batches/remaining_queue_web_review_search_queries.csv",
)
if batch_rows:
    with open(batch_rows[0]["batch_path"], "r", encoding="utf-8") as input_file:
        write_text_if_changed(input_file.read(), "../output/remaining_queue_web_review_batches/remaining_queue_web_review_next_batch.md")
