...
if (length(args) != 3) {
  stop("Expected 3 arguments: dob_open_data_raw_files_csv out_index_csv out_qc_csv")
}
...
for (i in seq_len(nrow(dob_raw_files))) {
  ...
  missing_bbl <- is.na(staged_df$bbl)
  staged_df$bbl[missing_bbl] <- build_bbl(staged_df$borough, staged_df$block, staged_df$lot)[missing_bbl]
