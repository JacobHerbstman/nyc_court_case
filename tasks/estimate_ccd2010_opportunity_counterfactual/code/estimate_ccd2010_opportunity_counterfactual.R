...
support_by_borough <- counterfactual_panel |>
  filter(spec_id == "main_A_PLUTO_WLS", outcome_margin == "units_built_total", period == "2010-2019") |>
  group_by(borough_code, borough_name) |>
  summarize(
    high_redev_n = n_distinct(district_id[high_redev_for_spec]),
    target_hh_high_redev_n = n_distinct(district_id[target_counterfactual]),
    low_homeowner_high_redev_n = first(reference_low_homeowner_high_redev_count),
    low_homeowner_any_n = first(reference_low_homeowner_count),
    reference_fallback_all_low_homeowner = first(reference_fallback_all_low_homeowner),
    treat_cf_for_spec_boro = first(treat_cf_for_spec_boro),
    treat_z_high_redev_min = min(treat_z_boro[high_redev_for_spec], na.rm = TRUE),
    treat_z_high_redev_max = max(treat_z_boro[high_redev_for_spec], na.rm = TRUE),
    treat_z_high_redev_sd = stats::sd(treat_z_boro[high_redev_for_spec], na.rm = TRUE),
    observed_units_target_2010_2019 = sum(observed_units[target_counterfactual], na.rm = TRUE),
    missing_units_target_2010_2019 = sum(missing_units_signed[target_counterfactual], na.rm = TRUE),
    .groups = "drop"
  ) |>
  arrange(borough_code)

main_city_totals <- counterfactual_summary |>
  filter(spec_id == "main_A_PLUTO_WLS", geography_level == "city") |>
  select(
    outcome_margin, period,
    city_target_district_count = target_district_count,
    city_observed_units = observed_units,
    city_counterfactual_units = counterfactual_units,
    city_missing_units_signed = missing_units_signed
  )

district_influence <- counterfactual_panel |>
  filter(spec_id == "main_A_PLUTO_WLS", target_counterfactual) |>
  select(
    outcome_margin, outcome_label, period, period_start, period_end, period_years,
    district_id, council_district, borough_code, borough_name,
    observed_units, units_counterfactual, missing_units_signed, missing_units_se
  ) |>
  left_join(main_city_totals, by = c("outcome_margin", "period"), relationship = "many-to-one") |>
  mutate(
    city_missing_units_after_excluding_district = city_missing_units_signed - missing_units_signed,
    city_observed_units_after_excluding_district = city_observed_units - observed_units,
    influence_share_of_city_missing = if_else(abs(city_missing_units_signed) > .Machine$double.eps, missing_units_signed / city_missing_units_signed, NA_real_)
  ) |>
  arrange(outcome_margin, period, desc(abs(influence_share_of_city_missing)), council_district)

matched_pairs <- bind_rows(lapply(seq_len(nrow(redev_df |> filter(high_homeowner, high_redev_A))), function(i) {
  target_df <- redev_df |>
    filter(high_homeowner, high_redev_A) |>
    arrange(council_district) |>
    slice(i)

  donor_pool <- redev_df |>
    filter(
      borough_code == target_df$borough_code[[1]],
      !high_homeowner,
      high_redev_A
    )
  ...
  donor_df <- donor_pool |>
    mutate(match_distance_redev_A = abs(redev_A_z_boro - target_df$redev_A_z_boro[[1]])) |>
    arrange(match_distance_redev_A, council_district) |>
    slice_head(n = 1)

  tibble(
    target_district_id = target_df$district_id,
    target_council_district = target_df$council_district,
    borough_code = target_df$borough_code,
    borough_name = target_df$borough_name,
    donor_district_id = donor_df$district_id,
    donor_council_district = donor_df$council_district,
    target_treat_z_boro = target_df$treat_z_boro,
    donor_treat_z_boro = donor_df$treat_z_boro,
    target_redev_A_z_boro = target_df$redev_A_z_boro,
    donor_redev_A_z_boro = donor_df$redev_A_z_boro,
    match_distance_redev_A = donor_df$match_distance_redev_A
  )
}))

matched_diagnostic <- matched_pairs |>
  left_join(
    analysis_df |>
      select(target_district_id = district_id, outcome_margin, period, target_observed_units = observed_units, target_rate = rate_annualized_per_10k_occ1990),
    by = "target_district_id",
    relationship = "one-to-many"
  ) |>
  left_join(
    analysis_df |>
      select(donor_district_id = district_id, outcome_margin, period, donor_observed_units = observed_units, donor_rate = rate_annualized_per_10k_occ1990),
    by = c("donor_district_id", "outcome_margin", "period"),
    relationship = "many-to-one"
  ) |>
  mutate(
    rate_gap_target_minus_donor = target_rate - donor_rate,
    observed_units_gap_target_minus_donor = target_observed_units - donor_observed_units
  ) |>
  arrange(target_council_district, outcome_margin, period)

duplicate_panel_keys <- counterfactual_panel |>
  count(spec_id, outcome_margin, district_id, period, name = "row_count") |>
  filter(row_count > 1)

duplicate_summary_keys <- counterfactual_summary |>
  count(spec_id, geography_level, borough_code, outcome_margin, period, name = "row_count") |>
  filter(row_count > 1)
...
qc_df <- bind_rows(
  tibble(
    section = "coverage",
    item = c(
      "redev_district_count",
      "analysis_district_count",
      "spec_count",
      "outcome_margin_count",
      "source_family_count",
      "panel_row_count",
      "summary_row_count"
    ),
    value = as.character(c(
      n_distinct(redev_df$district_id),
      n_distinct(analysis_df$district_id),
      n_distinct(counterfactual_panel$spec_id),
      n_distinct(counterfactual_panel$outcome_margin),
      n_distinct(counterfactual_panel$source_family),
      nrow(counterfactual_panel),
      nrow(counterfactual_summary)
    )),
    note = c(
      "Council districts in redevelopment-potential input.",
      "Council districts in period-outcome analysis panel.",
      "Counterfactual specifications in panel output.",
      "Outcome margins in panel output.",
      "Construction source families in panel output.",
      "Rows in district-period-outcome-spec panel.",
      "Rows in geography-period-outcome-spec summary."
    )
  ),
  tibble(
    section = "support",
    item = c(
      "main_A_min_low_homeowner_high_redev_reference_count",
      "main_A_max_low_homeowner_high_redev_reference_count",
      "main_A_reference_fallback_borough_count"
    ),
    value = as.character(c(
      min(counterfactual_panel$reference_low_homeowner_high_redev_count[counterfactual_panel$spec_id == "main_A_PLUTO_WLS"], na.rm = TRUE),
      max(counterfactual_panel$reference_low_homeowner_high_redev_count[counterfactual_panel$spec_id == "main_A_PLUTO_WLS"], na.rm = TRUE),
      n_distinct(counterfactual_panel$borough_code[counterfactual_panel$spec_id == "main_A_PLUTO_WLS" & counterfactual_panel$reference_fallback_all_low_homeowner])
    )),
    note = "Within-borough low-homeowner/high-redevelopment support for the main counterfactual reference."
  ),
  tibble(
    section = "missingness",
    item = c(
      ...
      "missing_observed_units",
      "target_rows_missing_signed_units",
      "target_rows_missing_se_main_wls"
    ),
    value = as.character(c(
      sum(is.na(analysis_df$treat_z_boro)),
      sum(is.na(analysis_df$redev_A_z_boro)),
      sum(is.na(analysis_df$observed_units)),
      sum(counterfactual_panel$target_counterfactual & is.na(counterfactual_panel$missing_units_signed)),
      sum(counterfactual_panel$target_counterfactual & counterfactual_panel$spec_id == "main_A_PLUTO_WLS" & is.na(counterfactual_panel$missing_units_se))
    )),
    note = "Missingness checks for treatment, opportunity, observed outcomes, and target counterfactual estimates."
  ),
  tibble(
    section = "uniqueness",
    item = c("duplicate_panel_keys", "duplicate_summary_keys"),
    value = as.character(c(nrow(duplicate_panel_keys), nrow(duplicate_summary_keys))),
    note = "Duplicate key checks for panel and summary outputs."
  ),
  tibble(
    section = "target_sample",
    item = c(
      "main_A_target_district_count",
      "main_A_high_redev_district_count",
      "main_A_high_homeowner_high_redev_district_count",
      "matched_pair_count"
    ),
    value = as.character(c(
      n_distinct(counterfactual_panel$district_id[counterfactual_panel$spec_id == "main_A_PLUTO_WLS" & counterfactual_panel$target_counterfactual]),
      sum(redev_df$high_redev_A, na.rm = TRUE),
      sum(redev_df$high_homeowner & redev_df$high_redev_A, na.rm = TRUE),
      nrow(matched_pairs)
    )),
    note = "Target and diagnostic matched sample counts for the main index A specification."
  ),
  tibble(
    section = "model_terms",
    item = c(
      "main_A_treatment_coefficient_count",
      "main_A_outcome_count_with_any_dropped_terms",
      "main_A_outcome_count_with_dropped_post_treatment_terms"
    ),
    value = as.character(c(
      nrow(main_treatment_coefficients),
      n_distinct(main_dropped_terms$outcome_margin[main_dropped_terms$has_dropped_terms]),
      n_distinct(main_dropped_terms$outcome_margin[main_dropped_terms$has_dropped_post_treatment_terms])
    )),
    note = "Dropped-term and treatment-coefficient checks for the main WLS specification."
  ),
  tibble(
    section = "diagnostic_outputs",
    item = c(
      "support_by_borough_rows",
      "district_influence_rows"
    ),
    value = as.character(c(
      nrow(support_by_borough),
      nrow(district_influence)
    )),
    note = "Rows in second-pass audit diagnostic outputs."
  ),
  tibble(
    section = "interpretation",
    item = "label",
    value = "descriptive opportunity-set counterfactual / welfare bridge",
    note = "Do not interpret as a clean causal estimate using redevelopment potential as a pre-treatment control."
  )
)
...
write_csv_if_changed(qc_df, "../output/ccdist2010_opportunity_counterfactual_qc.csv")
...
write_csv_if_changed(support_by_borough, "../output/ccdist2010_opportunity_counterfactual_support_by_borough.csv")
write_csv_if_changed(district_influence, "../output/ccdist2010_opportunity_counterfactual_district_influence.csv")
write_csv_if_changed(matched_diagnostic, "../output/ccdist2010_opportunity_counterfactual_matched_diagnostic.csv")
